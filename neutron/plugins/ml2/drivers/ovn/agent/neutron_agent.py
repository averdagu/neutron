...
class AgentCache:
    ...
    def agents_by_chassis_private(self, chassis_private):
        ...
        for cls in NeutronAgent.types.values():
            id_ = cls.id_from_chassis_private(chassis_private)
            try:
                agent = self.agents[id_]
            except KeyError:
                continue
            # NOTE (twilson) we are distinguishing between subclasses, so
            # isinstance() will not work. It would be simpler here if we went
            # back to not having ControllerGatewayAgent as separate cls, but
            # also more complex elsewhere.
            if type(agent) == cls:  # pylint: disable=unidiomatic-typecheck
                agents.add(agent)
        return agents
